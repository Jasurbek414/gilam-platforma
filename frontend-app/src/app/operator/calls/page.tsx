    ...
    <div className="flex flex-col gap-4 sm:gap-6 min-h-screen pb-10 font-sans">
        ...
        <div className={`flex items-center gap-2.5 px-4 py-2 border rounded-2xl shadow-sm ${
          ...
          <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.15em]">
            {sip.status === 'registered'  ? `SIP Ulangan ✓` :
             ...
             sip.status === 'error'       ? 'SIP ulanmagan' :
             'Ulanmagan'}
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 sm:gap-6 items-start">
        ...
        <div className="xl:col-span-4 bg-white/95 backdrop-blur-3xl border border-slate-200/50 rounded-3xl sm:rounded-[40px] shadow-2xl shadow-indigo-500/10 p-4 flex flex-col gap-3 sticky top-5 h-fit max-w-[300px] sm:max-w-[280px] mx-auto xl:mx-0 order-first xl:order-last">
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-indigo-600 flex items-center justify-center">
                <MdDialpad className="text-white text-[10px]" />
              </div>
              <h3 className="text-[8px] font-black text-slate-800 uppercase tracking-widest leading-none">Mini Dialer</h3>
            </div>
            
            <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full border text-[6px] font-black uppercase tracking-tighter ${
              ...
              <SipDot status={sip.status} />
              {sip.status === 'registered' ? 'Active' : 'Offline'}
            </div>
          </div>

          {/* Micro Display */}
          <div className="bg-slate-50/50 rounded-2xl border border-slate-100 px-3 py-2 flex items-center gap-2 transition-all focus-within:bg-white overflow-hidden">
            <div className="flex-1 flex flex-row-reverse overflow-x-auto scrollbar-hide py-0.5">
              <span className={`text-lg font-black font-mono tracking-tight tabular-nums whitespace-nowrap ${dialNum ? 'text-slate-800' : 'text-slate-200'}`}>
                {dialNum || '000'}
              </span>
            </div>
            {dialNum && (
              <button 
                ...
                className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-slate-300 hover:text-rose-500 transition-all active:scale-90"
              >
                <MdBackspace className="text-xs" />
              </button>
            )}
          </div>

          {/* Keypad - High Density */}
          <div className="grid grid-cols-3 gap-y-2.5 gap-x-2.5 px-1 pb-1">
            {KEYS.map(({ k, s }) => {
              const is0 = k === '0';
              return (
                <button key={k}
                  ...
                  className="relative aspect-square w-full max-w-[42px] mx-auto flex flex-col items-center justify-center rounded-full bg-slate-50 hover:bg-white border border-slate-100 hover:border-indigo-100 active:scale-90 active:bg-indigo-50 transition-all group"
                >
                  <span className="text-base font-black text-slate-800 group-active:text-indigo-600 leading-none">
                    {k}
                  </span>
                  {s && (
                    <span className="text-[5px] font-black text-slate-400 group-hover:text-indigo-400 tracking-tighter mt-0.5 uppercase">
                      {s}
                    </span>
                  )}
                  {is0 && (
                    <span className="absolute bottom-1 text-[6px] font-black text-indigo-200">+</span>
                  )}
                </button>
              );
            })}
          ...
          {/* Circular Action Buttons - The Ultimate Compactness */}
          <div className="flex items-center justify-center gap-4 pt-1 mb-1">
             {isActive ? (
               <button onClick={handleHangup}
                 className="w-12 h-12 bg-rose-600 hover:bg-rose-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-rose-200 active:scale-90 transition-all"
                 title="Tugatish"
               >
               ...
               <>
                 <button onClick={() => handleMakeCall(dialNum)}
                   ...
                   className="w-12 h-12 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-100 disabled:text-slate-300 text-white rounded-full flex items-center justify-center shadow-lg shadow-emerald-200 active:scale-90 transition-all group"
                   title="Qo'ng'iroq"
                 >
                   <MdCall className={`text-xl ${dialNum && sip.status === 'registered' ? 'animate-bounce' : ''}`} />
                 </button>
               </>
             )}
          </div>
