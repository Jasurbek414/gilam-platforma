...
export function useSip(_credentials?: SipCredentials | null) {
    ...
    socket.on(
      ...
      (data: {
        ...
        if (data.registered) {
          ...
          setStatus('error');
          setError(
            `SIP ulanmagan | Server: ${data.sipServer} | Local: ${data.localIp} | Ext: ${data.extension}`,
          );
        }
      },
    );
  ...
  const makeCall = useCallback(
    (target: string) => {
      ...
      if (socket?.connected) {
        ...
        toast.error('Backend bilan aloqa yo\'q. Sahifani yangilang.');
      }
    },
    [error],
  );
  ...
  const toggleMute = useCallback((_mute: boolean) => {
    // RTP darajasida amalga oshiriladi — keyingi bosqichda
  }, []);
